...
@dataclass
class CBISDDSMDataModuleConfig:
    dataroot: str = str("/data2/data/CBIS-DDSM/data/CBIS-DDSM-All-doiJNLP-zzWs5zfZ/CBIS-DDSM")
    split_file: str = str("/data2/data/CBIS-DDSM/uid/common_case_description_{split}_set.csv")
...
class CBISDDSMDataset(Dataset, Updateable):
    def __init__(self, cfg: CBISDDSMDataModuleConfig, split: str):
        super().__init__()
        ...
        image_names = image_data["image_mapped"].values.tolist()
    ...
    def __getitem__(self, index):
        ...
        if torch.any(img.isnan()):
            print(f"Warning, nan found for {img_path}")
...
@register("cbis-ddsm-datamodule")
class CBISDDSMDataModule(pl.LightningDataModule):
    cfg: dict[Literal["train", "val", "test"], CBISDDSMDataModuleConfig]
    ...
    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        ...
        self.cfg = {
            key: parse_structured(CBISDDSMDataModuleConfig, {**cfg, **conf_override}) 
            for key, conf_override in conf_overrides.items()
        }

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = CBISDDSMDataset(self.cfg["train"], "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = CBISDDSMDataset(self.cfg["val"], "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = CBISDDSMDataset(self.cfg["test"], "test")
