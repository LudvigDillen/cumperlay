...
from .cbis import CBISDDSMDataModule, CBISDDSMDataset

PERCENTS = [0.01, 0.05, 0.10, 0.20, 0.50]
OUTPUT_FILE = "./lmp/data/cbis/cbis_{split}_{ptext}.csv"
TRAIN_DATA_PERCENT_SEED = 0

def parse_dataset(dataset: CBISDDSMDataset):
...
for percent in PERCENTS:
    ...
    config = dict(
        dataroot="/data2/data/ck223/CBIS-DDSM",
        split_file="{dataroot}/common/common_case_description_{split}_mapped.csv",
        image_size=224,
        batch_size=32,
        workers=4,
        augment_extra=False,
    ...
    datamodule = CBISDDSMDataModule(config)
