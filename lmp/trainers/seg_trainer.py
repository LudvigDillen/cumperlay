...
@lmp.register("segmentation-trainer")
class SegmentationTrainer(BaseTrainer):
    ...
    class Config(BaseTrainer.Config):
        ...
        task: str = "multiclass" # "binary" | "multiclass" 
    ...
    def configure(self) -> None:
        ...
        self.train_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
        self.train_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
        self.train_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)

        self.val_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
        self.val_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
        self.val_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)

        self.test_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
        self.test_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
        self.test_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)

        if self.cfg.topo_pred:
            self.topo_train_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
            self.topo_train_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
            self.topo_train_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)

            self.topo_val_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
            self.topo_val_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
            self.topo_val_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)

            self.topo_test_acc = torchmetrics.classification.Accuracy(**cls_kwargs)
            self.topo_test_miou = torchmetrics.segmentation.MeanIoU(**seg_kwargs)
            self.topo_test_dice = torchmetrics.segmentation.DiceScore(**seg_kwargs)
    ...
    def loss(self, input_dict, phase="train"):
        ...
        if self.cfg.aux_pred:
            # (Binary) Cross entropy
            set_loss("aux_ce", self.ce_loss(out["aux"], target))
        if self.cfg.topo_pred:
            # (Binary) Cross entropy
            set_loss("topo_ce", self.ce_loss(out["topo"], target))

        if self.cfg.cfn_entropy and phase == "train":
        ...
        if self.cfg.multifilt_reg_loss:
            ...
            if len(layers) == 5: # and all_cfn[0].shape[-1] != all_cfn[-1].shape[-1]:
                ...
                all_cfn = all_cfn
            # N_F = all_cfn.shape[1]
            N_F = all_cfn[0].shape[1]

            mult_reg_loss_total = 0.0
            for i in range(N_F-1):
                for cfn in all_cfn:
                    cfn_curr = cfn[:, i:i+1]
                    cfn_next = cfn[:, i+1:i+2]
    ...
    def log_stats(self, output, phase="train"):
        ...
        # TODO: if self.cfg.aux_pred and "aux" in output
        if self.cfg.topo_pred and "topo" in output:
            self._log_stats({
                "pred": output["topo"],
    ...
    def _log_stats(self, output, phase="train", prefix=""):
        ...
        miou_fn = getattr(self, f"{prefix}{phase}_miou")
        dice_fn = getattr(self, f"{prefix}{phase}_dice")
        ...
        with torch.no_grad():
            ...
            if phase == "train":
                target = target.clone()
            target[mask] = self.cfg.num_classes
            target_one_hot = F.one_hot(target, num_classes=self.cfg.num_classes+1).movedim(-1, 1)
            target_one_hot = target_one_hot[:, :-1]
            pred = output["pred"]
            pred = F.one_hot(pred.argmax(dim=1),num_classes=self.cfg.num_classes).movedim(-1, 1)
        miou_fn(pred, target_one_hot)
        dice_fn(pred, target_one_hot)

        self.log(f"{phase}/{prefix}accuracy", acc_fn,
                 on_step=(phase == "train"), on_epoch=True)
        self.log(f"{phase}/{prefix}miou", miou_fn,
                 on_step=(phase == "train"), on_epoch=True)
        self.log(f"{phase}/{prefix}dice", dice_fn,
                 on_step=(phase == "train"), on_epoch=True)
