...
@lmp.register("network-lmp-swin-v2")
class LMPSwinV2(SwinV2):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.topo_net = nn.Sequential(
            ...
            nn.Linear(2 * self.topo_ch, self.num_classes)
        )
    ...
    def update(self):
        ...
        if self.input_guidance:
            ...
            if self.cfg.input_non_learned:
                ...
                self.input_topo = self._make_topo_layer(128, self.cfg.input_filt_rows, self.cfg.input_pers_chs, inp_f_args, inp_p_args, 1)

        gate_kwargs = {
            "bias": self.cfg.gate_bias
        }
        self.topo_1 = self._make_topo_layer(128, filt_rows[0], pers_chs[0], filtration_args[0], persistence_args[0], 1)
        self.linear_1 = self._make_combine_layer(topo_ch_mult * self.topo_ch, 128, **gate_kwargs)

        self.topo_2 = self._make_topo_layer(256, filt_rows[1], pers_chs[1], filtration_args[1], persistence_args[1], 2) 
        self.linear_2 = self._make_combine_layer(topo_ch_mult * self.topo_ch, 256, **gate_kwargs)

        self.topo_3 = self._make_topo_layer(512, filt_rows[2], pers_chs[2], filtration_args[2], persistence_args[2], 3) 
        self.linear_3 = self._make_combine_layer(topo_ch_mult * self.topo_ch, 512, **gate_kwargs)

        self.topo_4 = self._make_topo_layer(1024, filt_rows[3], pers_chs[3], filtration_args[3], persistence_args[3], 4)
        self.linear_4 = self._make_combine_layer(topo_ch_mult * self.topo_ch, 1024, **gate_kwargs)
